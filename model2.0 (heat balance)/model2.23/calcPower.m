...
%     pipe_pow = sum(arrayfun(@(x) powerfun(sim, sim.hot_water.temperature - x, sim.TE_material.seebeck, sim.pipe.dX), sim.temperatures(1,:)));
    pipe_elec_resistance = (sim.pipe.length/sim.pipe.dX)^2 * sim.TE_material.electrical_resistivity * sim.TE_material.thickness / (2*pi*sim.pipe.radius*sim.pipe.length);
    pipe_pow = pipe_voltage^2/(4*pipe_elec_resistance);
