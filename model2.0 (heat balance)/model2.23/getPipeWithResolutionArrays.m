function pipe = getPipeWithResolutionArrays(pipe, length_resolution)
    pipe.length_arr = linspace(0,pipe.length,length_resolution);
    pipe.dX = pipe.length_arr(2) - pipe.length_arr(1);
    pipe.length_resolution = length_resolution;
    
    initial_res = .0001 * pipe.radius;
    exp_base = 1.0001;
